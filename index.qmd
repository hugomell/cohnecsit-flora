...
- Basic info on volumes of articles
- Fig. ROSES flow diagram

### Study validity assessment

- Volumes critically appraised and reasons for high risk of bias
- Number of studies for narrative synthesis and meta-analyses and restate
  conditions to be included in each.

### Characteristics of the studies

- percent low risk of bias
- numbers by question
...
We extracted quantitative data from `r DATA_LIST$N_studies_meta-analysis` among
the `r DATA_LIST$N_studies_medium_low_bias` with low or medium risk of bias
...
LTIs?"). 
Among the `r DATA_LIST$N_cases_meta-analysis` extracted from these 
`r DATA_LIST$N_studies_meta-analysis`, 
`r DATA_LIST$N_cases_meta-analysis_Abundance` in 
`r DATA_LIST$N_studies_meta-analysis_Abundance` 
studies referred to species or group abundance, and 
`r DATA_LIST$N_cases_meta-analysis_SR` 
in `r DATA_LIST$N_studies_meta-analysis_SR` 
referred to species richness (Additional file ***XXX***).
...
`r DATA_LIST$N_cases_variance_imputation` 
(`r  paste0(DATA_LIST$N_cases_variance_imputation / DATA_LIST$N_cases_meta-analysis, "%")` 
of the total data).
When several cases were extracted from the same study, this was because there
was data for several species or group of species (
`r DATA_LIST$N_studies_meta-analysis_multiple_species`), for several sites (
`r DATA_LIST$N_studies_meta-analysis_multiple_sites`) or for several years or seasons 
(`r DATA_LIST$N_studies_meta-analysis_multiple_years_or_seasons`).

Most of the cases in our data were conducted along roads (highways 
`r paste0(DATA_LIST$prct_cases_meta-analysis_highways, "%")` and non-highway roads 
`r paste0(DATA_LIST$prct_cases_meta-analysis_nonhighways, "%")`
), then along powerlines (`r paste0(DATA_LIST$prct_cases_meta-analysis_powerlines, "%")`), pipelines (
`r paste0(DATA_LIST$prct_cases_meta-analysis_pipelines, "%")`) and railways (
`r paste0(DATA_LIST$prct_cases_meta-analysis_railways, "%")`).
***TODO: No studies for LTIs but I do not think relevant in our case ???***
Cases were mostly from ***TODO: percent countries*** 
`r paste0(DATA_LIST$prct_cases_meta-analysis_Europe, "%")`, 
`r paste0(DATA_LIST$prct_cases_meta-analysis_North_America, "%")`, 
`r paste0(DATA_LIST$prct_cases_meta-analysis_Oceania, "%")`, 
`r paste0(DATA_LIST$prct_cases_meta-analysis_South_America, "%")`, 
`r paste0(DATA_LIST$prct_cases_meta-analysis_Asia, "%")`, 
`r paste0(DATA_LIST$prct_cases_meta-analysis_Africa, "%")`  (?? - not sure)
**TODO: percent studies dealt with Native or Exotic studies**

### Meta-analyses and publication bias

For abundance, we found that, overall, vascular plants were **TODO: grand mean abundance**
(grand mean effect size `r paste0("d=", DATA_LIST$Effect_size_Abundance),` 95%
confidence interval (95% CI) 
`r patse0("[", DATA_LIST$Effect_size_Abundance_CI_inf, ";", DATA_LIST$Effect_size_Abundance_CI_supp, "]")`).
