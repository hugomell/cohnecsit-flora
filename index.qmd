...
We aimed to assess the effect of the type of LTI, as well as the 
*native* or *exotic* status of the flora studied on the effect sizes calculated
for abundance. We first analyzed the effect of the type of LTI using all the
non-redundant effect sizes extracted from the studies, regardless of plant
status. This means that we did not include effect sizes that were
calculated on a subset of the data used to compute another more general
one. So for instance, if effect sizes were available for ruderal plants
as well as native and non-native ruderals, we only used the effect size for
ruderals in this global analysis. We also discarded the data for railways as we
only had a single case for this LTI.
To see if the habitat role of verges was different for exotic and native
plants, we also ran a separate analysis using subsets of the data for each LTI
where we had effect sizes specifically for the abundance of native and/or
exotic species.
